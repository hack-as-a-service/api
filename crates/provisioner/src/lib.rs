...
impl Provisioner {
    ...
    pub async fn tarball_body_for_git_uri(uri: &Uri) -> Result<Body> {
        ...
        let mapped_stream = codec.framed(f)
            .map(|i| i
                .map(|b| b.freeze())
                // Has to be coerced for Into<Body>
                .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync + 'static>));
        // Type that Into<Body> expects
        let s2: Box<dyn Stream<Item = std::result::Result<hyper::body::Bytes, Box<dyn std::error::Error + Send + Sync + 'static>>> + Send + 'static> = Box::new(mapped_stream);
        Ok(s2.into())
    }

    pub async fn build_image_from_github(&mut self, image_id: &str, uri: &Uri) -> Result<impl Stream<Item = std::result::Result<bollard::models::BuildInfo, bollard::errors::Error>>> {
